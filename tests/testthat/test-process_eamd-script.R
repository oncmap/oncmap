test_that("process_eamd processing", {
  input_file <- "../../inst/extdata/sample-data-ecap2.csv"
  expect_equal(round(process_eamd(input_file)$adh,4),
  ...
  expect_equal(round(report$adh,4),
               data.frame(daysadh=0.375, daysdose=0.5833, perdoses=0.7917, n=24))

  expect_equal(round(process_eamd(input_file, regimen=data.frame(periods_per_day=1,doses_per_period=2,days_per_week=2,weekdays="0,1"))$adh,4),
